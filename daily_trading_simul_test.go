...
func TestS일일가격정보_저장소(t *testing.T) {
	...
	for i := 0; i < 기간; i++ {
		...
		공용.F테스트_에러없음(t, 에러)
		공용.F테스트_같음(t, 전일, 당일.AddDate(0, 0, -1))
	...
	공용.F테스트_다름(t, 익일_가격정보, nil)
	공용.F테스트_같음(t, 익일_가격정보.M시가, 100+10*int64(차이+1))
	공용.F테스트_같음(t, 익일_가격정보.M종가, 100+10*int64(차이+1)+10)
...
func TestS포트폴리오_매입_매도(t *testing.T) {
	...
	공용.F테스트_같음(t, len(포트폴리오.G일자별_코드_모음(초기_일자)), 2)
	공용.F테스트_같음(t, 포트폴리오.G구성요소(P현금_코드, 초기_일자).G평가액(), 500)
	공용.F테스트_같음(t, 포트폴리오.G구성요소(종목.G코드(), 초기_일자).G수량(), 5)
	공용.F테스트_같음(t, 포트폴리오.G구성요소(종목.G코드(), 초기_일자).G평가액(), 500)

	포트폴리오.S매도(종목.G코드(), 초기_일자, 5, 100)
	공용.F테스트_같음(t, len(포트폴리오.G일자별_코드_모음(초기_일자)), 1)
	공용.F테스트_같음(t, 포트폴리오.G구성요소(P현금_코드, 초기_일자).G평가액(), 1000)
	공용.F테스트_같음(t, 포트폴리오.G구성요소(종목.G코드(), 초기_일자), nil)
}
...
func TestS포트폴리오_상향돌파_하향돌파(t *testing.T) {
	...
	공용.F테스트_거짓임(t, 가격정보_저장소.G상향돌파(종목.G코드(), 초기_일자.AddDate(0, 0, 2), P장_개시, 90))
	공용.F테스트_거짓임(t, 가격정보_저장소.G상향돌파(종목.G코드(), 초기_일자.AddDate(0, 0, 2), P장_개시, 110))
	공용.F테스트_거짓임(t, 가격정보_저장소.G상향돌파(종목.G코드(), 초기_일자.AddDate(0, 0, 2), P장_개시, 130))

	공용.F테스트_거짓임(t, 가격정보_저장소.G상향돌파(종목.G코드(), 초기_일자.AddDate(0, 0, 2), P장_종료, 90))
	공용.F테스트_참임(t, 가격정보_저장소.G상향돌파(종목.G코드(), 초기_일자.AddDate(0, 0, 2), P장_종료, 110))
	공용.F테스트_거짓임(t, 가격정보_저장소.G상향돌파(종목.G코드(), 초기_일자.AddDate(0, 0, 2), P장_종료, 130))

	공용.F테스트_거짓임(t, 가격정보_저장소.G하향돌파(종목.G코드(), 초기_일자.AddDate(0, 0, 2), P장_개시, 95))
	공용.F테스트_참임(t, 가격정보_저장소.G하향돌파(종목.G코드(), 초기_일자.AddDate(0, 0, 2), P장_개시, 105))
	공용.F테스트_거짓임(t, 가격정보_저장소.G하향돌파(종목.G코드(), 초기_일자.AddDate(0, 0, 2), P장_개시, 115))

	공용.F테스트_거짓임(t, 가격정보_저장소.G하향돌파(종목.G코드(), 초기_일자.AddDate(0, 0, 2), P장_종료, 90))
	공용.F테스트_거짓임(t, 가격정보_저장소.G하향돌파(종목.G코드(), 초기_일자.AddDate(0, 0, 2), P장_종료, 110))
	공용.F테스트_거짓임(t, 가격정보_저장소.G하향돌파(종목.G코드(), 초기_일자.AddDate(0, 0, 2), P장_종료, 130))
}
