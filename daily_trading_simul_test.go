...
func TestS포트폴리오_당일_종가_익일_시가_매입_매도(t *testing.T) {
	...
	포트폴리오.S전량_매도(가격정보_저장소, 종목.G코드(), 초기_일자.AddDate(0, 0, 1), P장_개시) // 장 개시 시점에 판단하면 장 종료 시점에 매도.
	공용.F테스트_같음(t, 포트폴리오.G구성요소(P현금_코드, 초기_일자.AddDate(0, 0, 1)).G평가액(), 초기_현금.G평가액())
	공용.F테스트_같음(t, len(포트폴리오.G일자별_코드_모음(초기_일자.AddDate(0, 0, 1))), 1)
	...
	포트폴리오.S전량_매도(가격정보_저장소, 종목.G코드(), 초기_일자.AddDate(0, 0, 1), P장_종료) // 장 종료 시점에 판단하면 익일 장 개시 시점에 매도
	공용.F테스트_같음(t, 포트폴리오.G구성요소(P현금_코드, 초기_일자.AddDate(0, 0, 1)).G평가액(), 초기_현금.G평가액())
	공용.F테스트_같음(t, len(포트폴리오.G일자별_코드_모음(초기_일자.AddDate(0, 0, 1))), 1)
}
