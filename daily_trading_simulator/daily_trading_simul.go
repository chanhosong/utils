...
package daily_trading_simulator
...
func (s S일일가격정보_저장소) G종목별_일자_모음(종목코드 string) []time.Time {
	...
	return lib.F슬라이스_복사(lib.F정렬_시각(일자_모음), make([]time.Time, 0)).([]time.Time)
}
...
func (s S일일가격정보_저장소) g일자_도우미(일자_모음 []time.Time, 일자 time.Time, 일자_차이 int) (time.Time, error) {
	당일_인덱스 := 0
	for i, 일자값 := range 일자_모음 {
		if 일자.Equal(일자값) {
			당일_인덱스 = i
			break
		}
	...
	return time.Time{}, lib.New에러("당일을 찾을 수 없음. %v", 일자.Format(lib.P일자_형식))
}
...
func (s S일일가격정보_저장소) G종목별_과거_일자(종목코드 string, 기준일 time.Time, 일자_차이 int) (time.Time, error) {
	일자_차이 = int(lib.F2절대값(일자_차이))
	return s.g종목별_일자(종목코드, 기준일, 일자_차이*-1)
}

func (s S일일가격정보_저장소) G종목별_익일(종목코드 string, 기준일 time.Time) (time.Time, error) {
...
func (s S일일가격정보_저장소) G전종목_전일(일자 time.Time) (time.Time, error) {
	return s.G전종목_과거_일자(일자, 1)
}

func (s S일일가격정보_저장소) G전종목_과거_일자(일자 time.Time, 일자_차이 int) (time.Time, error) {
	일자_차이 = int(lib.F2절대값(일자_차이))
	return s.g전종목_일자(일자, 일자_차이*-1)
}

func (s S일일가격정보_저장소) G전종목_익일(일자 time.Time) (time.Time, error) {
	return s.G전종목_미래_일자(일자, 1)
}

func (s S일일가격정보_저장소) G전종목_미래_일자(일자 time.Time, 일자_차이 int) (time.Time, error) {
	일자_차이 = int(lib.F2절대값(일자_차이))
	return s.g전종목_일자(일자, 일자_차이)
}

func (s S일일가격정보_저장소) G당일_가격정보(종목코드 string, 일자 time.Time) *lib.S일일_가격정보 {
...
func (s S일일가격정보_저장소) G전일_가격정보(종목코드 string, 일자 time.Time) *lib.S일일_가격정보 {
	for {
		전일, 에러 := s.G종목별_전일(종목코드, 일자)
		if 에러 != nil {
			return nil
		}

		if 전일_가격정보 := s.G당일_가격정보(종목코드, 전일); 전일_가격정보 != nil {
			return 전일_가격정보.G복사본()
		}
	}
...
func (s S일일가격정보_저장소) G익일_가격정보(종목코드 string, 일자 time.Time) *lib.S일일_가격정보 {
	for {
		익일, 에러 := s.G종목별_익일(종목코드, 일자)
		if 에러 != nil {
			return nil
		}

		if 익일_가격정보 := s.G당일_가격정보(종목코드, 익일); 익일_가격정보 != nil {
			return 익일_가격정보.G복사본()
		}
	}
...
func (s S일일가격정보_저장소) G전일_종가(종목코드 string, 일자 time.Time) (int64, error) {
	전일_가격정보 := s.G전일_가격정보(종목코드, 일자)
	if 전일_가격정보 == nil {
		return -1, lib.New에러("전일 가격정보를 찾을 수 없습니다. %v", 일자.Format(lib.P일자_형식))
	}

	return 전일_가격정보.M조정종가, nil
}

func (s S일일가격정보_저장소) G당일_시가(종목코드 string, 일자 time.Time) (int64, error) {
	당일_가격정보 := s.G당일_가격정보(종목코드, 일자)
	if 당일_가격정보 == nil {
		return -1, lib.New에러("당일 가격정보를 찾을 수 없습니다. %v", 일자.Format(lib.P일자_형식))
	}

	return 당일_가격정보.G조정시가(), nil
}

func (s S일일가격정보_저장소) G당일_종가(종목코드 string, 일자 time.Time) (int64, error) {
	당일_가격정보 := s.G당일_가격정보(종목코드, 일자)
	if 당일_가격정보 == nil {
		return -1, lib.New에러("당일 가격정보를 찾을 수 없습니다. %v", 일자.Format(lib.P일자_형식))
	}

	return 당일_가격정보.M조정종가, nil
}

func (s S일일가격정보_저장소) G익일_시가(종목코드 string, 일자 time.Time) (int64, error) {
	익일_가격정보 := s.G익일_가격정보(종목코드, 일자)
	if 익일_가격정보 == nil {
		return -1, lib.New에러("익일 가격정보를 찾을 수 없습니다. %v", 일자.Format(lib.P일자_형식))
	}

	return 익일_가격정보.G조정시가(), nil
}

func (s S일일가격정보_저장소) G상향돌파(종목코드 string, 일자 time.Time,
	장_개시_종료 lib.T장_개시_종료, 기준단가 int64) bool {
	전일_가격정보 := s.G전일_가격정보(종목코드, 일자)
	당일_가격정보 := s.G당일_가격정보(종목코드, 일자)

	switch {
	case 전일_가격정보 == nil, 당일_가격정보 == nil:
		return false // 가격정보를 찾을 수 없음.
	case 장_개시_종료 == lib.P장_개시 &&
		전일_가격정보.M조정종가 <= 기준단가 &&
		당일_가격정보.G조정시가() > 기준단가:
		return true // 시가 상향 돌파
	case 장_개시_종료 == lib.P장_종료 &&
		당일_가격정보.G조정시가() <= 기준단가 &&
		당일_가격정보.M조정종가 > 기준단가:
		return true // 종가 상향 돌파
	}

	return false
}

func (s S일일가격정보_저장소) G하향돌파(종목코드 string, 일자 time.Time, 장_개시_종료 lib.T장_개시_종료,
	기준단가 int64) bool {
	전일_가격정보 := s.G전일_가격정보(종목코드, 일자)
	당일_가격정보 := s.G당일_가격정보(종목코드, 일자)
	...
	case 장_개시_종료 == lib.P장_개시 &&
		전일_가격정보.M조정종가 >= 기준단가 &&
		당일_가격정보.G조정시가() < 기준단가:
		return true // 시가 하향 돌파
	case 장_개시_종료 == lib.P장_종료 &&
		당일_가격정보.G조정시가() >= 기준단가 &&
		당일_가격정보.M조정종가 < 기준단가:
		return true // 종가 하향 돌파
	}
...
func (s S일일가격정보_저장소) G단순이동평균(종목코드 string, 기준일 time.Time,
	기간by일 int, 가격종류 lib.T가격데이터_구분) (float64, error) {
	가격정보_일자_모음 := s.G전종목_일자_모음()
	switch {
	case 기간by일 <= 0:
		return 0, lib.New에러("잘못된 기간입니다. %v", 기간by일)
	case len(가격정보_일자_모음) < 기간by일,
		기준일.Before(s.G전종목_일자_모음()[기간by일]):
		return 0, lib.New에러("가격정보가 충분하지 않습니다. %v", 기준일.Format(lib.P일자_형식))
	}
	...
	for i := 0; i < 기간by일; i++ {
		var 가격정보 *lib.S일일_가격정보

		// 시간을 거슬러 올라가면서 가격정보를 취득함.
		for {
			가격정보 = s.G당일_가격정보(종목코드, 일자)
			if 가격정보 != nil {
				break
			}
...
func (s *S일일가격정보_저장소) S추가(일일_가격정보 *lib.S일일_가격정보) {
...
func New일일가격정보_저장소() *S일일가격정보_저장소 {
	s := new(S일일가격정보_저장소)
	s.맵 = make(map[string]*lib.S일일_가격정보)
...
type S포트폴리오 struct {
	...
	포트폴리오_코드_일자_키 map[string](map[time.Time]lib.S비어있음)
	포트폴리오_일자_코드_키 map[time.Time](map[string]lib.S비어있음)
	거래내역_모음       []*S거래내역
}
...
func (s S포트폴리오) G일자별_코드_모음(일자 time.Time) []string {
	코드_맵, ok := s.포트폴리오_일자_코드_키[lib.F2일자(일자)]
	if !ok {
		코드_맵 = make(map[string]lib.S비어있음)
	}
...
func (s S포트폴리오) G전종목_거래내역_모음() []*S거래내역 {
	return lib.F슬라이스_복사(s.거래내역_모음, make([]*S거래내역, 0)).([]*S거래내역)
}
...
func (s *S포트폴리오) S전일_복제(기준일 time.Time, 가격정보_저장소 *S일일가격정보_저장소) (에러 error) {
	...
	lib.F조건부_패닉(코드_모음 == nil, "예상하지 못한 경우")

	for _, 종목코드 := range 코드_모음 {
		전일_조정종가 := int64(0)
		전일_가격정보 := 가격정보_저장소.G당일_가격정보(종목코드, 전일)
		당일_구성요소 := s.G구성요소(종목코드, 전일) // 복제

		switch {
		case 종목코드 == P현금_코드:
			전일_조정종가 = 당일_구성요소.G평가액()
		case 전일_가격정보 != nil:
			전일_조정종가 = 전일_가격정보.M조정종가
		default:
			lib.F패닉("전일종가를 알 수 없어서 평가액을 계산할 수 없습니다.")
		}

		// 필요한 부분 업데이트
		당일_구성요소.일자 = 기준일
		당일_구성요소.평가액 = 당일_구성요소.수량 * 전일_조정종가
		당일_구성요소.키 = f키(종목코드, 기준일)
...
반복문:
	for _, 미결_거래내역 := range 미결_거래내역_모음 {
		...
		case 미결_거래내역.G수량() > 잔여_수량:
			lib.F패닉("'S거래내역'은 1회에 전량 매도하는 것을 가정하여 설계되었습니다.")
		case 미결_거래내역.G수량() == 잔여_수량:
			미결_거래내역.S매도기록(일자, 단가)
			break 반복문
		case 미결_거래내역.G수량() < 잔여_수량:
...
func (s *S포트폴리오) S종목별_손절매(가격정보_저장소 *S일일가격정보_저장소, 종목코드 string,
	일자 time.Time, 장_개시_종료 lib.T장_개시_종료) bool {
	구성요소 := s.G구성요소(종목코드, 일자)
	if 구성요소 == nil {
		return false
	}

	당일_가격정보 := 가격정보_저장소.G당일_가격정보(종목코드, 일자)
	if 당일_가격정보 == nil {
		return false
	}
	...
	for _, 거래내역 := range 거래내역_모음 {
		switch {
		case 거래내역.G매입일().After(일자):
			continue
		case 장_개시_종료 == lib.P장_개시:
			if 당일_가격정보.G조정시가() > 거래내역.G손절매_단가() {
				continue
			}

			s.S전량_매도_당일_종가(가격정보_저장소, 종목코드, 일자)
			손절매_수행 = true
		case 장_개시_종료 == lib.P장_종료:
			if 당일_가격정보.M조정종가 > 거래내역.G손절매_단가() {
				continue
			}

			s.S전량_매도_익일_시가(가격정보_저장소, 종목코드, 일자)
			손절매_수행 = true
		default:
			lib.F패닉("예상하지 못한 경우.")
		}
...
func (s *S포트폴리오) S전종목_손절매(가격정보_저장소 *S일일가격정보_저장소, 일자 time.Time,
	장_개시_종료 lib.T장_개시_종료) {
	일자 = lib.F2일자(일자)

	종목코드_모음 := s.G일자별_코드_모음(일자)
	for _, 종목코드 := range 종목코드_모음 {
		s.S종목별_손절매(가격정보_저장소, 종목코드, 일자, 장_개시_종료)
	}
}

func (s *S포트폴리오) S당일_종가_매입(가격정보_저장소 *S일일가격정보_저장소, 종목코드 string, 일자 time.Time, 수량 int64, 손절매_비율 float64) {
	매입_단가, 에러 := 가격정보_저장소.G당일_종가(종목코드, 일자)
	lib.F에러2패닉(에러)
	손절매_단가 := int64(float64(매입_단가) * (100.0 - 손절매_비율) / 100.0)
	s.S매입(종목코드, 일자, 수량, 매입_단가, 손절매_단가)
}

func (s *S포트폴리오) S익일_시가_매입(가격정보_저장소 *S일일가격정보_저장소, 종목코드 string, 일자 time.Time, 수량 int64, 손절매_비율 float64) {
	매입_단가, 에러 := 가격정보_저장소.G익일_시가(종목코드, 일자)
	lib.F에러2패닉(에러)
	손절매_단가 := int64(float64(매입_단가) * (100.0 - 손절매_비율) / 100.0)
	s.S매입(종목코드, 일자, 수량, 매입_단가, 손절매_단가)
}

func (s *S포트폴리오) S당일_종가_매도(가격정보_저장소 *S일일가격정보_저장소, 종목코드 string, 일자 time.Time, 수량 int64) {
	매도_단가, 에러 := 가격정보_저장소.G당일_종가(종목코드, 일자)
	lib.F에러2패닉(에러)
	s.S매도(종목코드, 일자, 수량, 매도_단가)
}

func (s *S포트폴리오) S익일_시가_매도(가격정보_저장소 *S일일가격정보_저장소, 종목코드 string, 일자 time.Time, 수량 int64) {
	매도_단가, 에러 := 가격정보_저장소.G익일_시가(종목코드, 일자)
	lib.F에러2패닉(에러)
	s.S매도(종목코드, 일자, 수량, 매도_단가)
}
...
func (s *S포트폴리오) S전량_매도_당일_종가(가격정보_저장소 *S일일가격정보_저장소, 종목코드 string, 일자 time.Time) {
	매도_단가, 에러 := 가격정보_저장소.G당일_종가(종목코드, 일자)
	lib.F에러2패닉(에러)
	s.S전량_매도(종목코드, 일자, 매도_단가)
}

func (s *S포트폴리오) S전량_매도_익일_시가(가격정보_저장소 *S일일가격정보_저장소, 종목코드 string, 일자 time.Time) {
	매도_단가, 에러 := 가격정보_저장소.G익일_시가(종목코드, 일자)
	lib.F에러2패닉(에러)
	s.S전량_매도(종목코드, 일자, 매도_단가)
}
...
func New포트폴리오() *S포트폴리오 {
	...
	s.포트폴리오_코드_일자_키 = make(map[string](map[time.Time]lib.S비어있음))
	s.포트폴리오_일자_코드_키 = make(map[time.Time](map[string]lib.S비어있음))
...
func (s *S포트폴리오_구성요소) s매입(수량, 단가 int64) {
	...
	case P현금_코드:
		if 수량 != 1 {
			lib.F패닉("현금은 수량을 1로 해야 합니다. %v", 수량)
		}
...
func (s *S포트폴리오_구성요소) s매도(수량, 단가 int64) {
	switch {
	case P현금_코드 == s.종목코드 && 수량 != 1:
		lib.F패닉("현금은 수량을 1로 해야 합니다. %v", 수량)
	case 수량 > s.수량:
		lib.F패닉("매도 수량이 보유 수량보다 더 큽니다. %v %v", s.수량, 수량)
	case P현금_코드 == s.종목코드:
		s.수량 = 수량
		s.평가액 -= 단가
	default:
...
func (s S포트폴리오_구성요소) String() string {
	...
	버퍼.WriteString(", ")
	버퍼.WriteString(s.일자.Format(lib.P일자_형식))
	버퍼.WriteString(", ")
...
func (s S거래내역) G거래완료() bool {
	return !s.매도일.Equal(time.Time{}) || s.매도_단가 != 0
}
func (s *S거래내역) S매도기록(매도일 time.Time, 매도_단가 int64) {
	switch {
	case 매도일.Before(s.매입일):
		lib.F패닉("잘못된 일자. %v %v", s.매입일.Format(lib.P일자_형식), s.매도일.Format(lib.P일자_형식))
	case 매도_단가 <= 0:
		lib.F패닉("잘못된 매도 단가. %v", s.매도_단가)
	}
