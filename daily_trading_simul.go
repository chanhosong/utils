...
func (s S일일가격정보_저장소) g일자_도우미(일자_모음 []time.Time, 당일 time.Time, 일자_차이 int) (time.Time, error) {
	당일_인덱스 := 0
	...
	return time.Time{}, 공용.New에러("당일을 찾을 수 없음. %v", 당일.Format(공용.P일자_형식))
}
...
func (s S일일가격정보_저장소) G전종목_전일(당일 time.Time) (time.Time, error) {
	return s.G전종목_과거_일자(당일, 1)
}

func (s S일일가격정보_저장소) G전종목_과거_일자(당일 time.Time, 일자_차이 int) (time.Time, error) {
	일자_차이 = int(공용.F2절대값(일자_차이))
	return s.g전종목_일자(당일, 일자_차이*-1)
}

func (s S일일가격정보_저장소) G전종목_익일(당일 time.Time) (time.Time, error) {
	return s.G전종목_미래_일자(당일, 1)
}

func (s S일일가격정보_저장소) G전종목_미래_일자(당일 time.Time, 일자_차이 int) (time.Time, error) {
	일자_차이 = int(공용.F2절대값(일자_차이))
	return s.g전종목_일자(당일, 일자_차이)
}
...
func (s S일일가격정보_저장소) G전일_가격정보(종목코드 string, 당일 time.Time) *공용.S일일_가격정보 {
	전일 := 당일
...
func (s S일일가격정보_저장소) G익일_가격정보(종목코드 string, 당일 time.Time) *공용.S일일_가격정보 {
	익일 := 당일
...
func (s *S포트폴리오) S종목별_손절매(가격정보_저장소 *S일일가격정보_저장소, 종목코드 string,
	당일 time.Time, 장_개시_종료 uint8) bool {
	구성요소 := s.G구성요소(종목코드, 당일)
	if 구성요소 == nil {
		return false
	}

	당일_가격정보 := 가격정보_저장소.G당일_가격정보(종목코드, 당일)
	if 당일_가격정보 == nil {
		return false
	}
	...
	for _, 거래내역 := range 거래내역_모음 {
		...
		case 장_개시_종료 == P장_종료:
			...
			익일_가격정보 := 가격정보_저장소.G익일_가격정보(종목코드, 당일)
			if 익일_가격정보 == nil {
				continue
			}

			s.S전량_매도(거래내역.G종목코드(), 당일, 익일_가격정보.G조정시가())
			손절매_수행 = true
		default:
			공용.F패닉("예상하지 못한 경우.")
		}
...
func (s *S포트폴리오) S전종목_손절매(가격정보_저장소 *S일일가격정보_저장소, 당일 time.Time, 장_개시_종료 uint8) {
	당일 = 공용.F2일자(당일)

	종목코드_모음 := s.G일자별_코드_모음(당일)
	for _, 종목코드 := range 종목코드_모음 {
		s.S종목별_손절매(가격정보_저장소, 종목코드, 당일, 장_개시_종료)
	}
}

func (s *S포트폴리오) S당일_종가_매입(가격정보_저장소 *S일일가격정보_저장소, 종목코드 string, 당일 time.Time, 수량 int64, 손절매_비율 float64) {
	당일_가격정보 := 가격정보_저장소.G당일_가격정보(종목코드, 당일)
	공용.F조건부_패닉(당일_가격정보 == nil, "당일 가격정보를 찾을 수 없습니다. %v", 당일.Format(공용.P일자_형식))
	매입_단가 := 당일_가격정보.M조정종가
	손절매_단가 := int64(float64(매입_단가) * (100.0 - 손절매_비율) / 100.0)
	s.S매입(종목코드, 당일, 수량, 매입_단가, 손절매_단가)
}

func (s *S포트폴리오) S익일_시가_매입(가격정보_저장소 *S일일가격정보_저장소, 종목코드 string, 당일 time.Time, 수량 int64, 손절매_비율 float64) {
	익일, 에러 := 가격정보_저장소.G종목별_익일(종목코드, 당일)
	공용.F에러_패닉(에러)
	익일_가격정보 := 가격정보_저장소.G당일_가격정보(종목코드, 익일)
	공용.F조건부_패닉(익일_가격정보 == nil, "익일 가격정보를 찾을 수 없습니다. %v", 익일.Format(공용.P일자_형식))
	매입_단가 := 익일_가격정보.G조정시가()
	손절매_단가 := int64(float64(매입_단가) * (100.0 - 손절매_비율) / 100.0)
	s.S매입(종목코드, 당일, 수량, 매입_단가, 손절매_단가)
}

func (s *S포트폴리오) S당일_종가_매도(가격정보_저장소 *S일일가격정보_저장소, 종목코드 string, 당일 time.Time, 수량 int64) {
	당일_가격정보 := 가격정보_저장소.G당일_가격정보(종목코드, 당일)
	공용.F조건부_패닉(당일_가격정보 == nil, "당일 가격정보를 찾을 수 없습니다. %v", 당일.Format(공용.P일자_형식))
	매도_단가 := 당일_가격정보.M조정종가
	s.S매도(종목코드, 당일, 수량, 매도_단가)
}

func (s *S포트폴리오) S익일_시가_매도(가격정보_저장소 *S일일가격정보_저장소, 종목코드 string, 당일 time.Time, 수량 int64) {
	익일, 에러 := 가격정보_저장소.G종목별_익일(종목코드, 당일)
	공용.F에러_패닉(에러)
	익일_가격정보 := 가격정보_저장소.G당일_가격정보(종목코드, 익일)
	공용.F조건부_패닉(익일_가격정보 == nil, "익일 가격정보를 찾을 수 없습니다. %v", 익일.Format(공용.P일자_형식))
	매도_단가 := 익일_가격정보.M조정종가
	s.S매도(종목코드, 익일, 수량, 매도_단가)
}
