...
func (s *S포트폴리오) S종목별_손절매(가격정보_저장소 *S일일가격정보_저장소, 종목코드 string,
	...
	for _, 거래내역 := range 거래내역_모음 {
		...
		case 장_개시_종료 == P장_종료:
			...
			s.s전량_매도_단순형(거래내역.G종목코드(), 당일, 익일_가격정보.G조정시가())
			손절매_수행 = true
		default:
			공용.F패닉("예상하지 못한 경우.")
		}
...
func (s *S포트폴리오) s전량_매도_단순형(종목코드 string, 일자 time.Time, 단가 int64) {
	수량 := s.G수량(종목코드, 일자)
	s.S매도(종목코드, 일자, 수량, 단가)
}

func (s *S포트폴리오) S전량_매도(가격정보_저장소 *S일일가격정보_저장소, 종목코드 string, 일자 time.Time, 장_개시_종료 uint8) {
	switch 장_개시_종료 {
	case P장_개시:
		당일_가격정보 := 가격정보_저장소.G당일_가격정보(종목코드, 일자)
		공용.F조건부_패닉(당일_가격정보 == nil, "당일 가격정보를 찾을 수 없습니다. %v", 일자.Format(공용.P일자_형식))
		매도_단가 := 당일_가격정보.M조정종가
		s.s전량_매도_단순형(종목코드, 일자, 매도_단가)
	case P장_종료:
