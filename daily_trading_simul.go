...
func (s S일일가격정보_저장소) G전일_가격정보(종목코드 string, 금일 time.Time) *공용.S일일_가격정보 {
	...
	for i:=0 ; i<10 ; i++ {
		일일_가격정보, ok := s.맵[f키(종목코드, 전일)]
		if ok {
			return 일일_가격정보.G복사본()
		}

		if 전일, 에러 = s.G전일(전일); 에러 != nil {
			return nil
		}
	}
...
func (s S일일가격정보_저장소) G명일_가격정보(종목코드 string, 금일 time.Time) *공용.S일일_가격정보 {
	...
	for i:=0 ; i<10 ; i++ {
		일일_가격정보, ok := s.맵[f키(종목코드, 명일)]
		if ok {
			return 일일_가격정보.G복사본()
		}

		if 명일, 에러 = s.G명일(명일); 에러 != nil {
			return nil
		}
	}
...
func New현금(일자 time.Time, 금액 int64) *S포트폴리오_구성요소 {
	...
	s.수량 = 1
	s.평가액 = 금액
	s.키 = 현금_코드 + "_" + s.일자.Format("20060102")
...
func (s S거래기록) G거래완료() bool { return s.매도일 == time.Time{} && s.매도_단가 == 0 }
func (s *S거래기록) S매도일(매도일 time.Time) {
	if 매도일.Before(s.매입일) {
...
func (s *S거래기록) S매도_단가(매도_단가 int64)  {
	if 매도_단가 <= 0 {
