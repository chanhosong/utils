...
func (s S일일가격정보_저장소) g일자_도우미(일자_모음 []time.Time, 일자 time.Time, 일자_차이 int) (time.Time, error) {
	...
	case 일자_차이 < 0 && 당일_인덱스 < 일자_차이*-1,
		...
		return time.Time{}, 공용.F에러_생성("데이터 없음. %v %v", 당일_인덱스, 일자_차이)
	default:
		return 일자_모음[당일_인덱스+일자_차이], nil
	}

	return time.Time{}, 공용.New에러("당일을 찾을 수 없음. %v", 일자.Format(공용.P일자_형식))
}
...
func (s S일일가격정보_저장소) G당일_가격정보(종목코드 string, 일자 time.Time) *공용.S일일_가격정보 {
...
func (s S일일가격정보_저장소) G전일_가격정보(종목코드 string, 일자 time.Time) *공용.S일일_가격정보 {
	for {
...
func (s S일일가격정보_저장소) G익일_가격정보(종목코드 string, 일자 time.Time) *공용.S일일_가격정보 {
	for {
...
func (s S일일가격정보_저장소) G전일_종가(종목코드 string, 일자 time.Time) (int64, error) {
	...
	if 전일_가격정보 == nil {
		return -1, 공용.F에러_생성("전일 가격정보를 찾을 수 없습니다. %v", 일자.Format(공용.P일자_형식))
	}
...
func (s S일일가격정보_저장소) G당일_시가(종목코드 string, 일자 time.Time) (int64, error) {
	...
	if 당일_가격정보 == nil {
		return -1, 공용.F에러_생성("당일 가격정보를 찾을 수 없습니다. %v", 일자.Format(공용.P일자_형식))
	}
...
func (s S일일가격정보_저장소) G익일_시가(종목코드 string, 일자 time.Time) (int64, error) {
	...
	if 익일_가격정보 == nil {
		return -1, 공용.F에러_생성("익일 가격정보를 찾을 수 없습니다. %v", 일자.Format(공용.P일자_형식))
	}
...
func (s *S포트폴리오_구성요소) s매도(수량, 단가 int64) {
	...
	case P현금_코드 == s.종목코드 && 수량 != 1:
		공용.F패닉("현금은 수량을 1로 해야 합니다. %v", 수량)
	case 수량 > s.수량:
		공용.F패닉("매도 수량이 보유 수량보다 더 큽니다. %v %v", s.수량, 수량)
	case P현금_코드 == s.종목코드:
		s.수량 = 수량
		s.평가액 -= 단가
	default:
