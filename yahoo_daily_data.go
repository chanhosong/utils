...
func F종목별_일일가격정보_질의(종목 *공용.S종목) ([]*공용.S일일_종목정보, *공용.S에러내역) {
	...
	종목별_일일가격정보_모음, 에러내역 := 종목별_일일가격정보_모음_생성_도우미(종목, 일일가격정보_문자열_모음)
	if 에러내역 != nil {
		return nil, 에러내역
	}
...
func 종목별_일일가격정보_모음_생성_도우미(
	...
	가격정보_모음 := make([]*공용.S일일_종목정보, 0)
	const 날짜형식 = "2006-01-02"

	for _, 레코드 := range 레코드_모음 {
		일자 := 공용.F2포맷된_일자_단순형(날짜형식, 레코드[0])
		시가 := 공용.F2정수64_단순형(레코드[1])
		고가 := 공용.F2정수64_단순형(레코드[2])
		저가 := 공용.F2정수64_단순형(레코드[3])
		종가 := 공용.F2정수64_단순형(레코드[4])
		거래량 := 공용.F2정수64_단순형(레코드[5])
		조정종가 := 공용.F2정수64_단순형(레코드[6])


		일일가격정보 := new(공용.S일일_종목정보)
		일일가격정보.M일자 = 일자
		일일가격정보.M종목코드 = 종목.G코드()
		일일가격정보.M고가 = 고가
		일일가격정보.M저가 = 저가
