...
func F전종목_일일가격정보_확보_야후() (map[string]([]*lib.S일일_가격정보), map[string]*lib.S에러내역) {
	...
	for 반복횟수 := 0; 반복횟수 < 3; 반복횟수++ {
		for _, 종목 := range 종목모음_질의대상 {
			if 종목.G시장구분() != lib.P시장구분_ETF &&
				종목.G시장구분() != lib.P시장구분_코스피 {
				continue
			}

			lib.F문자열_출력("종목코드 %v, 종목명칭 %v", 종목.G코드(), 종목.G이름())
	...
	for _, 종목 := range 종목모음_에러발생 {
		...
		lib.F문자열_출력("%v : %v %v", 종목, 에러내역.G에러_코드(), 에러내역.G에러_메시지())
	}

	lib.F파일에_값_저장(가격정보_맵, P가격정보_맵_파일명, M가격정보_맵_파일_잠금)

	return 가격정보_맵, 에러내역_맵
}

func F종목별_일일가격정보_질의(종목 *lib.S종목) ([]*lib.S일일_가격정보, *lib.S에러내역) {
	시작일 := lib.F2포맷된_일자_단순형("2006-01-02", "1900-01-01")
	종료일 := lib.F2포맷된_일자_단순형("2006-01-02", time.Now().Format("2006-01-02"))
...
func 종목별_일일가격정보_질의_도우미_야후(종목 *lib.S종목, 시작일 time.Time, 종료일 time.Time) ([][]string, *lib.S에러내역) {
	...
	case 에러 != nil:
		lib.F체크_포인트()
		return nil, lib.New에러내역("HTTP요청 에러", 에러.Error())
	case 응답.Body == nil:
		lib.F체크_포인트()
		return nil, lib.New에러내역("nil 응답", "응답.Body가 nil입니다.")
	}
	...
	switch {
	case 에러 != nil:
		return nil, lib.New에러내역("CSV읽기 에러", 에러.Error()+"\n")
	case len(레코드_모음) < 1:
		// 가격데이터가 없거나, CSV로 해석불가능하면 무시
		return nil, lib.New에러내역("CSV읽기 에러", "CSV 레코드 수량이 0개")
	}

	lib.F변수값_확인(응답, 응답.Body, 레코드_모음)
...
func 야후_가격정보_질의_URL생성(종목 *lib.S종목, 시작일 time.Time, 종료일 time.Time) string {
	var 시장구분 string

	switch 종목.G시장구분() {
	case lib.P시장구분_코스피, lib.P시장구분_ETF:
		시장구분 = ".KS" // 유가증권 시장
	case lib.P시장구분_코스닥:
		시장구분 = ".KQ" // 코스닥 시장
	}
...
func 종목별_일일가격정보_모음_생성_도우미(종목 *lib.S종목, 레코드_모음 [][]string) (
	...
	defer func() {
		if r := recover(); r != nil {
			종목별_가격정보_모음 = nil
			에러내역 = lib.New에러내역("일일가격정보 생성 에러", lib.F포맷된_문자열("%", r))
		}
	}()
	...
	for _, 레코드 := range 레코드_모음 {
		일자 := lib.F2포맷된_일자_단순형(날짜형식, 레코드[0])
		시가 := int64(lib.F2실수_단순형(레코드[1]))
		고가 := int64(lib.F2실수_단순형(레코드[2]))
		저가 := int64(lib.F2실수_단순형(레코드[3]))
		종가 := int64(lib.F2실수_단순형(레코드[4]))
		거래량 := int64(lib.F2실수_단순형(레코드[5]))
		조정종가 := int64(lib.F2실수_단순형(레코드[6]))

		일일가격정보 := new(lib.S일일_가격정보)
		일일가격정보.M일자 = 일자
		일일가격정보.M종목코드 = 종목.G코드()
		일일가격정보.M고가 = 고가
		일일가격정보.M저가 = 저가
