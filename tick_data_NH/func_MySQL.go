...
func f실시간_데이터_수집_NH_ETF_MySQL(종목코드_모음 []string) (db *sql.DB, 에러 error) {
	...
	lib.F체크포인트("f실시간_데이터_수집_NH_ETF_MySQL()")

	ch수신 := make(chan lib.I소켓_메시지, 10000)
	ch초기화 := make(chan lib.T신호)

	lib.F체크포인트("nh.F실시간_데이터_구독_NH_ETF() 호출 시작. 호출 중 에러 발생함.")

	nh.F실시간_데이터_구독_NH_ETF(ch수신, 종목코드_모음)    // 여기에서 에러 발생함.

	lib.F체크포인트("nh.F실시간_데이터_구독_NH_ETF() 호출 완료")

	defer nh.F실시간_데이터_해지_NH_ETF(종목코드_모음)

	lib.F체크포인트("nh.Go루틴_실시간_정보_중계_MySQL() 호출 시작. 여기에서 응답이 없음.")

	go nh.Go루틴_실시간_정보_중계_MySQL(ch초기화)
	신호 := <-ch초기화

	lib.F체크포인트("nh.Go루틴_실시간_정보_중계_MySQL() 호출 완료")

	lib.F조건부_패닉(신호 != lib.P신호_초기화, "예상하지 못한 신호. %v", 신호)

	lib.F체크포인트("go루틴_실시간_데이터_수집_MySQL() 호출 시작")

	go go루틴_실시간_데이터_수집_MySQL(ch초기화, ch수신)
	신호 = <-ch초기화

	lib.F체크포인트("go루틴_실시간_데이터_수집_MySQL() 호출 완료")
